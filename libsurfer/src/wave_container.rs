...
pub enum SignalId {
    ...
    /// A decoded signal, see [`crate::decoders`]. The payload is a hash of the
    /// variable reference so the id can still be used as a cache key.
    Virtual(u64),
}
...
fn extract_index(s: &str) -> (String, Option<i64>) {
...
impl WaveContainer {
    ...
    /// Register a decoded signal, see [`crate::decoders`].
    pub fn add_virtual_signal(
        &mut self,
        name: String,
        signal: crate::decoders::VirtualSignal,
    ) -> Result<VariableRef> {
        match self {
            WaveContainer::Wellen(f) => Ok(f.add_virtual_signal(name, signal)),
            _ => bail!("Decoded signals are only supported for waveform files"),
        }
    }

    #[must_use]
    pub fn is_virtual(&self, variable: &VariableRef) -> bool {
        match self {
            WaveContainer::Wellen(f) => f.is_virtual(variable),
            _ => false,
        }
    }
    ...
    pub fn signal_id(&self, variable: &VariableRef) -> Result<SignalId> {
        match self {
            WaveContainer::Wellen(f) if f.is_virtual(variable) => {
                use std::hash::{Hash, Hasher};
                let mut h = std::collections::hash_map::DefaultHasher::new();
                variable.hash(&mut h);
                Ok(SignalId::Virtual(h.finish()))
            }
    ...
    pub fn is_signal_loaded(&self, signal_id: &SignalId) -> bool {
        match (self, signal_id) {
            ...
            (WaveContainer::Wellen(_), SignalId::Virtual(_)) => true,
            _ => false,
        }
    }
...
mod tests {
    ...
    fn extract_index_with_valid_index() {
        let (name, index) = extract_index("signal[5]");
    ...
    fn extract_index_with_zero_index() {
        let (name, index) = extract_index("data[0]");
        assert_eq!(name, "data");
    ...
    fn extract_index_with_negative_index() {
        let (name, index) = extract_index("array[-1]");
    ...
    fn extract_index_with_large_number() {
        let (name, index) = extract_index("mem[999999]");
        assert_eq!(name, "mem");
    ...
    fn extract_index_no_brackets() {
        let (name, index) = extract_index("simple_signal");
    ...
    fn extract_index_empty_brackets() {
        let (name, index) = extract_index("signal[]");
        assert_eq!(name, "signal[]");
        assert_eq!(index, None);
    }
    ...
    fn extract_index_non_numeric_index() {
        let (name, index) = extract_index("signal[abc]");
    ...
    fn extract_index_only_closing_bracket() {
        let (name, index) = extract_index("signal5]");
    ...
    fn extract_index_multiple_brackets() {
        let (name, index) = extract_index("array[3][5]");
    ...
    fn extract_index_with_dot_notation() {
        let (name, index) = extract_index("struct.field[10]");
    ...
    fn extract_index_bracket_at_start() {
        let (name, index) = extract_index("[5]signal");
    ...
    fn extract_index_no_text() {
        let (name, index) = extract_index("[5]");
        assert_eq!(name, "[5]");
        assert_eq!(index, None);
    }
}
