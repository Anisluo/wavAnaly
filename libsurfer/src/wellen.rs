...
use crate::decoders::VirtualSignal;
...
pub struct WellenContainer {
    ...
    /// Decoded / computed signals that are not part of the waveform file
    virtual_signals: HashMap<VariableRef, Arc<VirtualSignal>>,
...
impl WellenContainer {
    ...
    pub fn query_variable(
        ...
        if let Some(v) = self.virtual_signals.get(variable) {
            return Ok(Some(v.query(time)));
        }
        let h = &self.hierarchy;
    ...
    /// Register a decoded signal. It becomes queryable through the returned
    /// [`VariableRef`] (scope `decoded`).
    pub fn add_virtual_signal(&mut self, name: String, signal: VirtualSignal) -> VariableRef {
        let scope = ScopeRef::from_hierarchy_string("decoded");
        let variable = VariableRef::new(scope, name);
        self.virtual_signals
            .insert(variable.clone(), Arc::new(signal));
        variable
    }

    #[must_use]
    pub fn is_virtual(&self, variable: &VariableRef) -> bool {
        self.virtual_signals.contains_key(variable)
    }

    pub fn variable_to_meta(&self, variable: &VariableRef) -> Result<VariableMeta> {
        if let Some(v) = self.virtual_signals.get(variable) {
            return Ok(VariableMeta {
                var: variable.clone(),
                num_bits: None,
                variable_type: Some(VariableType::VCDString),
                variable_type_name: Some(v.type_name.clone()),
                index: None,
                direction: None,
                enum_map: HashMap::new(),
                encoding: VariableEncoding::String,
            });
        }
